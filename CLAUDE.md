...
- `add_fetish()` で新しい性癖を追加。現在の回答から事後確率が最も高い既存性癖を自動でテンプレートに使用。追加時はlearn×4回分のブーストを適用（learn_countは1回のみカウント）
...
| POST | /api/add_fetish | 新しい性癖を追加。`confirmed=true` で重複・類似チェックをスキップして確定 |
