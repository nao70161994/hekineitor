...
Broad heavy-emotion results (`共依存`, `激重感情`, `共生関係`, `執着`) have a factor cap of `0.55` and can be pushed down to `0.35` when recent exposure is still concentrated. A category quota applies on the latest 300 results: if heavy-emotion results exceed `10%`, their factor is capped at `0.25`; if they exceed `25%`, their factor is capped at `0.12`. While that hard quota is active, a heavy-emotion result must have at least `2.5x` the best non-heavy posterior in the adjustment pool; otherwise its effective factor is capped at `0.02`. A clearly dominant heavy-emotion result is allowed back up to `0.55`. Dominant top-result protection is disabled, so overexposed heavy-emotion results can lose close races even when they start as the top posterior candidate.
