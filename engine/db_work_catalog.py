...
def replace_catalog(cur, catalog, *, execute_values):
    ...
    batches = (
        ...
        (
            'INSERT INTO work_identity_reviews (review_id, review_type, candidate_key, work_ids, titles, asins, status) VALUES %s',
            [
                (
                    row['review_id'],
                    row['review_type'],
...
def migrate_legacy_catalog(cur, *, compound_data, execute_values):
    ...
    ensure_schema(cur)
    cur.execute("SELECT pg_advisory_xact_lock(hashtext('recommended_work_catalog_migration'))")
