...
`scripts/check.sh`ではPython compile、安全性check、Ruff lint/format、段階導入したmypy、pytestとcoverage最低基準、ESLint、Vitestを実行します。Playwrightの最小browser E2Eは`npm run test:e2e`で実行し、CIでも検証します。pytest実行時は`tests/conftest.py`が`FETISH_LOG_PATH`を一時ディレクトリへ向け、通常のテスト実行で本番/開発用の診断ログを汚さないようにしています。

Pythonの設定とcoverage閾値は`pyproject.toml`、JavaScriptのcommandと固定versionは`package.json` / `package-lock.json`にあります。Python本番依存は`requirements.in`から`requirements.txt`、開発依存は`requirements-dev.in`から`requirements-dev.txt`へ固定しています。詳しい検証方針は[`docs/QA.md`](docs/QA.md)を参照してください。
...
- `services/`: OGP、share、context、admin helper、admin security、server session、rate limit、response hooks、matrix backups、quality stats、ID parsing、app metadata、name matching、inference/learning/question selection facade。
- `engine/`: `__init__.py`を公開入口とする推論・学習・matrix操作package。`facade.py`が互換facadeを提供します。
...
- `docs/README.md`: 現行文書、ADR、履歴文書の索引。
...
- `tests/js/`はVitestで主要なclient状態遷移、`tests/browser/`はPlaywrightで代表導線を確認します。
- 自動検証と手動QAの境界は`docs/QA.md`と`docs/LIGHTWEIGHT_E2E.md`を参照してください。
