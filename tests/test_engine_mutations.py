...
class TestEngineMutations(unittest.TestCase):
    ...
    def test_review_manifest_local_mutation_commits_complete_resolved_catalog(self):
        ...
        before = {'fetishes': fetishes, 'work_catalog': catalog}
    ...
    def test_identifier_admin_operations_commit_catalog_atomically(self):
        ...
        state = {'fetishes': fetishes, 'work_catalog': catalog}
    ...
    def test_v3_restore_uses_catalog_and_discards_legacy_inline_works(self):
        ...
        exported = [
            {
                'id': fetish_id,
                'name': self.engine.fetishes[0]['name'],
                'desc': self.engine.fetishes[0]['desc'],
                'works': ['Legacy work'],
            }
        ]
        ...
        with (
            ...
            self.engine.restore_matrix_snapshot(exported, [], work_catalog=catalog)

        after = commit_catalog.call_args.args[1]
        self.assertNotIn('compound_works', after)
        self.assertEqual(after['work_catalog'], catalog)
        self.assertTrue(all('works' not in row for row in after['fetishes']))

    def test_db_v3_restore_delegates_catalog_without_legacy_projection(self):
        catalog = engine_module.work_catalog.build_catalog_from_inline(
            [{'id': self.engine.fetishes[0]['id'], 'works': ['Catalog work']}]
        )
        with (
            patch.object(engine_module, '_use_db', return_value=True),
            patch.object(
            ...
            self.engine.restore_matrix_snapshot(self.engine.fetishes, [], work_catalog=catalog)

        self.assertEqual(restore_db.call_args.kwargs['work_catalog'], catalog)
        self.assertNotIn('inline_corrections', restore_db.call_args.kwargs)
        self.assertNotIn('legacy_projection_fetishes', restore_db.call_args.kwargs)

    def test_correction_failure_does_not_publish_partial_catalog(self):
        catalog = engine_module.work_catalog.build_catalog_from_inline([{'id': 1, 'works': ['Before']}])
        before = {'fetishes': [], 'work_catalog': catalog}
        with (
            patch.object(engine_module, '_use_db', return_value=False),
            ...
            patch.object(engine_module.work_catalog, 'apply_catalog_corrections', return_value=catalog),
