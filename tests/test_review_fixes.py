...
class ReviewFixTests(unittest.TestCase):
    ...
    def test_workflow_warning_title_is_stable_and_restore_matches_retention(self):
        ...
        self.assertIn("plan = str(data.get('plan', '')).lower()", backup)
        self.assertIn("if plan != 'free':", backup)
        self.assertIn("expires_str = data.get('expiresAt', '')", backup)
        self.assertIn("'gh', 'issue', 'close'", backup)
        self.assertNotIn('days_left = 90 - days_old', backup)
