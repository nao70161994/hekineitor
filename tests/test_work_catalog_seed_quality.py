...
def test_existing_reviewed_catalog_migrates_to_exact_fresh_seed():
    ...
    target_id = next(
        row["work_id"] for row in legacy["works_master"] if row["canonical_title"] == "作品名"
    )
    decisions = {
        "schema_version": 1,
...
def test_checked_in_seed_has_only_verified_safe_normalizations():
    ...
    for expected in overrides["title_normalizations"]:
        matching_aliases = [
            row for row in aliases.values() if row["alias"] == expected["display_title"]
        ]
        ...
        matching_links = [
            row
            for row in links
            if row.get("alias_id") in {alias["alias_id"] for alias in matching_aliases}
        ]
    ...
    for key, works in compound_source.items():
        assert [row["title"] for row in materialized_compounds.get(key, [])] == [
            work_title(row) for row in works
        ]

    canonical_titles = {row["canonical_title"] for row in masters.values()}
    assert "現実で30歳独身・無職、仮想現実でリア充（参考）" in canonical_titles
    assert "賭ケグルイ（参考）" not in {
        row["display_title"] for row in overrides["title_normalizations"]
    }
