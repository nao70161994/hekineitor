...
class RestoreMatrixWorkflowTests(unittest.TestCase):
    ...
    def test_workflow_fetches_csrf_and_can_download_backup_artifact(self):
        ...
        self.assertRegex(workflow, r'uses: actions/download-artifact@v\d+')
        self.assertIn('name: matrix-backup-${{ github.event.inputs.backup_run_id }}', workflow)
        self.assertIn('run-id: ${{ github.event.inputs.backup_run_id }}', workflow)
        self.assertIn('github-token: ${{ secrets.GITHUB_TOKEN }}', workflow)
        self.assertIn('path: data', workflow)
